...
JDatePicker.directive('jMdDatepicker', function($timeout, $filter, $mdDialog, $compile, mdThemeColors, $mdMedia, JDatePickerCount) {
  return {
	...
	<label ng-attr-for="jmddpcomp-{{serial}}"><md-icon ng-show="{{search}}" md-svg-src="resources/img/icons/ic_search_24px.svg"></md-icon>{{placeholder}}</label>\
...
background-color : {{mdThemeColors.primary[\'800\']}} !important;\
color: #fff !important;\
}\
div.displaypanel div.titleheader {\
background-color : {{mdThemeColors.primary[\'800\']}};\
}\
div.displaypanel {\
background-color: {{mdThemeColors.primary[\'800\']}};\
}\
div.selectionpanel div.titleheader .md-button.navbutton div.leftarrow {\
border-right-color: {{mdThemeColors.primary[\'800\']}};\
}\
div.selectionpanel div.titleheader .md-button.navbutton div.rightarrow {\
border-left-color: {{mdThemeColors.primary[\'800\']}};\
}\
div.monthpanel .md-button.jmd-fab.today {\
color: {{mdThemeColors.primary[\'800\']}};\
}\
...
div.popupDialogContent {\
	padding: 0;\
      ...
      $scope.$watch('SelectedDate', function(newValue) {
        if (newValue != null) {
          $scope.SelectedDateText = $filter('date')(newValue, 'dd/MM/yyyy');
        } else {
...
JDatePicker.directive('jMdDatepickerComponent2', ['$timeout', '$compile', 'mdThemeColors', '$mdMedia', function($timeout, $compile, mdThemeColors, $mdMedia) {
  return {
    ...
    link: function($scope, $element, $attr, $ctrl) {
      ...
      $scope.ShortMonths =  ['JAN', 'FEV', 'MAR', 'ABR','MAI','JUN','JUL','AGO','SET','OUT','NOV','DEZ'];
    ...
    <div class="displaypanel">\
	  ...
	  <div class="datelabel">{{selDate | date:\'EEEE, MMMM d\'}}</div>\
    </div>\
\
    <div class="selectionpanel">\
	  ...
	  <div class="actionpanel">\
		...
		<md-button ng-click="CancelClick($event)">Cancelar</md-button>\
		<md-button ng-click="NowClick($event)">Hoje</md-button>\
	  </div>\
    </div>\
\
  </div>';
